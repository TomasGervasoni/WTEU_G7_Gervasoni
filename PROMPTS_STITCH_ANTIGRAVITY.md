...
| Pagos (2.5) | ✅ Cerrado y probado | CU-020 a CU-023. Integración con MercadoPago probada con credenciales de **producción** y pago real de $3 ARS (plata efectivamente movida entre cuentas, confirmado). Redirección a `init_point`/`sandbox_init_point` controlada por `MP_ENV` (production/sandbox), independiente de `NODE_ENV` (que debe quedar en `development` para que las cookies del login sigan funcionando en Docker local por HTTP — no confundir ambas variables). Webhook automático (CU-021/022 vía notificación de MP) **no funciona en local** — requiere URL pública (ngrok o Railway), documentado como pendiente conocido; la aprobación de pagos de MercadoPago se hace manualmente desde `PagosAdmin.html` (CU-022). Decisión consciente: los intentos de pago abandonados (usuario entra al checkout y no completa) quedan acumulados como 'pendiente' sin limpieza automática — se evaluó y se descartó por tiempo, no es un olvido. |
| Dashboard (2.6) | ✅ Cerrado | CU-024 a CU-028. Métricas agregadas viven en cada Service dueño (PedidosServices.obtenerMetricasPedidos, ClientesServices.obtenerMetricasClientes, PagosServices.obtenerEficienciaPagos), DashboardServices orquesta sin duplicar SQL (cross-module correcto). Gráfico de barras (ventas 6 meses) y donut (pedidos por estado) con SVG nativo manipulado por JS. Export a Excel con `exceljs`. Los KPIs de `PedidosAdminListado.html` reutilizan el mismo endpoint `/api/v1/dashboard` en vez de duplicar lógica. **TODOS LOS 28 CU DE LA DOCUMENTACIÓN IMPLEMENTADOS.** |

### Pendientes conocidos y decisiones de diseño (para mencionar si preguntan en la defensa)
- Webhook automático de MercadoPago (CU-021/022): no funciona en local, requiere URL pública (ngrok/Railway). Aprobación de pagos MP se hace manualmente vía `PagosAdmin.html`.
- Falta validar la firma `x-signature` del webhook con el secret de MP (solo relevante si el webhook llega a exponerse públicamente).
- Intentos de pago con MercadoPago abandonados (usuario entra al checkout y no completa) quedan acumulados como 'pendiente' sin limpieza automática — decisión consciente por tiempo, no un olvido.
- Sin control de stock/inventario en Pedidos: decisión de diseño correcta, el negocio es impresión DTF bajo demanda; "Gestión de Insumos" y "Producción" son módulos evolutivos fuera de alcance (sección 1.2 del documento). `producto_variantes.stock` existe en el esquema pero no se usa para ningún control.
- Puerto de Postgres expuesto en 5433 en el host (no 5432) por conflicto con un PostgreSQL nativo de Windows — el `.env` del contenedor `api` usa 5432 (interno, red Docker); 5433 es solo para clientes externos como DBClient.
